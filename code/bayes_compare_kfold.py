...
OUTPUT_DIR = RESULTS_ROOT / 'bayes_compare_kfold'

K_FOLDS = int(os.environ.get("K_FOLDS", "10"))
DRAWS = int(os.environ.get("PM_DRAWS", "200"))
TUNE = int(os.environ.get("PM_TUNE", "200"))
...
def fit_weibull(train):
    ...
    with pm.Model() as model:
        ...
        c = pm.Deterministic("c", pm.math.invlogit(pm.Normal("c_logit", mu_c, 1.0, shape=n_models)))
...
def elpd_weibull(idata, test):
    ...
    c = idata.posterior["c"].values.reshape(-1, idata.posterior["c"].shape[-1])

    p = c[:, model_idx] * np.exp(-((lam[:, model_idx] * t) ** k[:, model_idx]))
    p = np.clip(p, 1e-12, 1 - 1e-12)

    ll = y * np.log(p) + (1 - y) * np.log(1 - p)
    # log-mean-exp per observation
...
def elpd_exponential(idata, test):
    ...
    p = c[:, model_idx] * np.exp(-(lam[:, model_idx] * t))
    p = np.clip(p, 1e-12, 1 - 1e-12)
