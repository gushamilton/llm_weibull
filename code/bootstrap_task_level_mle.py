...
MIN_TASKS = int(os.environ.get("PETO_MIN_TASKS", "20"))
MIN_RUNS = int(os.environ.get("PETO_MIN_N", "50"))

EXCLUDE_ALIASES = {"gpt-3.5-turbo-instruct"}


def _mask_positive_time(t: np.ndarray, y: np.ndarray, task_ids: np.ndarray):
    mask = np.isfinite(t) & (t > 0)
    return t[mask], y[mask], task_ids[mask]
...
def main():
    ...
    required_cols = ["alias", "task_id", "human_minutes", "score_binarized"]
    ...
    for alias in aliases:
        ...
        task_ids = subset["task_id"].to_numpy(dtype=str)
        t, y, task_ids = _mask_positive_time(t, y, task_ids)
        ...
        for idx, tid in enumerate(task_ids):
            task_to_indices.setdefault(tid, []).append(idx)

        task_t = []
        task_y = []
        for tid in tasks:
            idxs = task_to_indices[tid]
            task_t.append(t[idxs])
            task_y.append(y[idxs])
        ...
        # Bootstrap by task: sample task IDs with replacement
        b_log_b0 = []
        b_log_b1 = []
        b_weib_lam = []
        b_weib_k = []
        ...
        for _ in range(BOOTSTRAP_N):
            sample_idx = rng.integers(0, n_tasks, size=n_tasks)
            tb = np.concatenate([task_t[i] for i in sample_idx], axis=0)
            yb = np.concatenate([task_y[i] for i in sample_idx], axis=0)
            ...
            if log_b.success and weib_b.success:
                ...
                # Akaike/BIC weights for two-model comparison
    ...
    out_csv = OUTPUT_DIR / "weibull_logistic_task_bootstrap_ci.csv"
    out.to_csv(out_csv, index=False)

    plot_bic = OUTPUT_DIR / "task_bootstrap_bic_diff.png"
    plot_aic = OUTPUT_DIR / "task_bootstrap_aic_diff.png"
    ...
    if not plot_df.empty:
        _plot_diff_ci(
            ...
            "bic_diff",
            plot_bic,
            "Task-level Bootstrap: BIC(Logistic) - BIC(Weibull)",
        )
    ...
    if not plot_df.empty:
        _plot_diff_ci(
            ...
            "aic_diff",
            plot_aic,
            "Task-level Bootstrap: AIC(Logistic) - AIC(Weibull)",
        )
